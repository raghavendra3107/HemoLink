...
const DonorDonationHistory = () => {
  ...
  return (
    <div className="min-h-screen relative p-6">
      {/* Immersive background image wrapper */}
      <div 
        className="fixed inset-0 z-0 bg-cover bg-center bg-no-repeat"
        style={{ backgroundImage: `url('/images/donation_history_bg.png')` }}
      >
                  ...
                  >
                    <div className="flex flex-row lg:items-center justify-between gap-4">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-600 mt-2">
                            ...
                            {(item.facility || item.city) && (
                              <p className="flex items-center gap-2">
                                ...
                                <span>
                                  {item.facility ? <span className="font-semibold">{item.facility}</span> : "Donation Camp"}
                                  {item.city && ` - ${item.city}`}
                      ...
                      <div className="flex flex-col items-center justify-center gap-1">
                        <div className="min-w-[4rem] h-10 px-3 bg-gradient-to-r from-red-500 to-pink-500 rounded-full flex items-center justify-center text-white font-bold text-md shadow-md">
                          +{item.quantity || 1}
                        </div>
                        <p className="text-xs text-gray-500 font-medium">
                           {item.quantity > 50 ? "ml" : "Units"}
                        </p>
                      </div>
                    </div>
                  </div>
