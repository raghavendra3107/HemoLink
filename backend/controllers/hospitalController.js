  ...
  try {
    ...
    if (availability !== "all") {
      ...
      if (availability === "available") {
        filter.$or = [
          { lastDonationDate: { $lt: sixtyDaysAgo } },
    ...
    // Calculate stats
    const sixtyDaysAgoStats = new Date();
    sixtyDaysAgoStats.setDate(sixtyDaysAgoStats.getDate() - 60);

    const [availableDonors, rareBloodDonors] = await Promise.all([
      Donor.countDocuments({
        $or: [
          { lastDonationDate: { $lt: sixtyDaysAgoStats } },
          { lastDonationDate: { $exists: false } }
        ]
      }),
      Donor.countDocuments({
