...
import CampRegistration from "../models/campRegistrationModel.js";
...
export const getDonorProfile = async (req, res) => {
  try {
    ...
    // Next eligible date (60 days rule)
...
export const markDonation = async (req, res) => {
  try {
    ...
    // Check if donor can donate (60 days gap)
    if (donor.lastDonationDate) {
      const lastDonation = new Date(donor.lastDonationDate);
      const now = new Date();
      const diffInDays = Math.floor((now - lastDonation) / (1000 * 60 * 60 * 24));
      
      if (diffInDays < 60) {
        const daysLeft = 60 - diffInDays;
        return res.status(400).json({ 
          success: false, 
          message: `Donor cannot donate yet. Minimum 60 days required between donations. Wait ${daysLeft} more days.` 
        });
      }
    }
...
export const registerForCamp = async (req, res) => {
  try {
    const donorId = req.user?.id || req.donor?._id || req.donor?.id || req.user?._id;
    const campId = req.params.id;
    const { timeSlot, quantityML, donationDate } = req.body;
    ...
    const existingRegistration = await CampRegistration.findOne({ donor: donorId, camp: campId });
    ...
    if (camp.status === "Cancelled" || camp.status === "Completed") {
       return res.status(400).json({ success: false, message: `Cannot register as the camp is ${camp.status}` });
    }

    const donor = await Donor.findById(donorId);
    if (!donor) return res.status(404).json({ success: false, message: "Donor not found" });

    // Enforce 60-day gap rule based on proposed donation date
    if (donor.lastDonationDate) {
      const lastDonation = new Date(donor.lastDonationDate);
      const proposedDate = donationDate ? new Date(donationDate) : new Date(camp.date);
      const diffInDays = Math.floor((proposedDate - lastDonation) / (1000 * 60 * 60 * 24));
      
      if (diffInDays < 60) {
        const daysLeft = 60 - diffInDays;
        return res.status(400).json({ 
          success: false, 
          message: `You are not eligible to donate on this date. You must wait ${daysLeft} more days (Minimum 60 days required).` 
        });
      }
    }
    ...
    const registration = await CampRegistration.create({
      ...
      camp: campId,
      timeSlot,
      quantityML: quantityML || 350,
      donationDate: donationDate ? new Date(donationDate) : new Date(camp.date),
    });

    // Optionally increment expectedDonors or actualDonors in BloodCamp?
    // Let's increment actualDonors when they actually donate, expectedDonors is capacity.

    res.status(201).json({ success: true, message: "Successfully registered for the blood camp", registration });
  } catch (error) {
...
/**
 * @desc Get all camp registrations for a donor
 * @route GET /api/donor/registrations
 * @access Private (Donor)
 */
export const getDonorRegistrations = async (req, res) => {
  try {
    const donorId = req.user?.id || req.donor?._id || req.donor?.id || req.user?._id;
    
    const registrations = await CampRegistration.find({ donor: donorId })
      .populate({
         path: 'camp',
         populate: { path: 'hospital', select: 'name facilityType address' }
      })
      .sort({ createdAt: -1 });

    res.status(200).json({ success: true, registrations });
  } catch (error) {
    console.error("Fetch Donor Registrations Error:", error);
...
export const getCampRegistrations = async (req, res) => {
  try {
    const campId = req.params.id;
    // Check if camp belongs to this lab (optional security check)
    // const labId = req.user._id;

    const registrations = await CampRegistration.find({ camp: campId })
      .populate('donor', 'fullName email phone bloodGroup lastDonationDate age weight gender')
      .sort({ createdAt: -1 });

    res.status(200).json({ success: true, registrations });
  } catch (err) {
    console.error("Get camp registrations error:", err);
    res.status(500).json({ success: false, message: "Server error" });
  }
};
 ...
 * @route PUT /api/blood-lab/camps/registrations/:id/status
 * @access Private (Blood Lab)
 */
export const updateRegistrationStatus = async (req, res) => {
  try {
    const registrationId = req.params.id;
    const { status, quantity, remarks } = req.body;
    const labId = req.user._id;

    const registration = await CampRegistration.findById(registrationId).populate('donor camp');
    if (!registration) {
      return res.status(404).json({ success: false, message: "Registration not found" });
    }

    if (registration.status === "Donated" && status !== "Donated") {
      return res.status(400).json({ success: false, message: "Cannot revert a verified donation through this endpoint" });
    }

    // If marking as donated
    if (status === "Donated" && registration.status !== "Donated") {
        const donor = registration.donor;
        const bloodGroup = donor.bloodGroup; // Assuming donor has defined blood group

        if (!bloodGroup) {
            return res.status(400).json({ success: false, message: "Donor blood group not registered."});
        }
        ...
        // Add to donation history using findByIdAndUpdate to avoid validation errors on selected: false fields
        ...
        await Facility.findByIdAndUpdate(labId, {
          $push: {
            history: {
              eventType: "Donation",
              description: `Recorded camp donation from ${donor.fullName} - ${quantity || 1} unit(s) of ${bloodGroup}`,
              date: new Date(),
              referenceId: donor._id,
            },
          },
        ...
        // Add to blood stock
        await addToBloodStock(labId, bloodGroup, quantity || 1);
        
        // Update camp actual donors count safely
        await BloodCamp.findByIdAndUpdate(registration.camp._id, {
           $inc: { actualDonors: 1 }
        });
    }
    ...
    res.status(200).json({ success: true, message: `Status updated to ${status}`, registration });
  } catch (err) {
